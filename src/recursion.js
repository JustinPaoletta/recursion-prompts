...
var arraySum = function(array) {
    let copy = [...array];
    let flatArray = copy.flat(Infinity);
    if (flatArray.length === 0) {
        return 0;
    } else {
        return flatArray.pop() + arraySum(flatArray);
    }
};
...
var rMap = function(array, callback) {
    ...
    if (cArr.length === 0) {
        ...
        let shifted = cArr.shift();
        myMap.push(callback(shifted));
        return myMap.concat(rMap(cArr, callback));
    }
};
...
var countKeysInObj = function(obj, key) {
    ...
    for (let keyx in obj) {
        ...
        if (typeof obj[keyx] === 'object' && !Array.isArray(obj[keyx])) {
            count += countKeysInObj(obj[keyx], key);
        }
    }
...
var countValuesInObj = function(obj, value) {
    ...
    for (let keyx in obj) {
        ...
        if (typeof obj[keyx] === 'object' && !Array.isArray(obj[keyx])) {
            count += countValuesInObj(obj[keyx], value);
        }
    }
